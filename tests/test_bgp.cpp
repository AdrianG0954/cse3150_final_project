...
{
    Announcement ann("192.168.1.0/24", {200}, 200, Relationship::CUSTOMER);
...
TEST_F(BGPTest, PrependingMultipleHops)
{
    ...
    Announcement ann("10.0.0.0/8", {200, 300, 400}, 200, Relationship::PEER);
    bgp->enqueueAnnouncement(ann);
    bgp->processAnnouncements();
...
TEST_F(BGPTest, ConflictResolution_CustomerBeatsProvider)
{
    ...
    Announcement providerAnn("192.168.1.0/24", {200}, 200, Relationship::PROVIDER);
    bgp->enqueueAnnouncement(providerAnn);
    bgp->processAnnouncements();

    // Second announcement from customer (should win)
    Announcement customerAnn("192.168.1.0/24", {300}, 300, Relationship::CUSTOMER);
...
TEST_F(BGPTest, ConflictResolution_PeerBeatsProvider)
{
    ...
    Announcement peerAnn("192.168.1.0/24", {300}, 300, Relationship::PEER);
    bgp->enqueueAnnouncement(peerAnn);
    bgp->processAnnouncements();

    const auto &rib = bgp->getlocalRib();
    const Announcement &stored = rib.at("192.168.1.0/24");
    EXPECT_EQ(Relationship::PEER, stored.getRelationship());
}

TEST_F(BGPTest, ConflictResolution_OriginBeatsAll)
{
    // First announcement from customer
    Announcement customerAnn("192.168.1.0/24", {200}, 200, Relationship::CUSTOMER);
    bgp->enqueueAnnouncement(customerAnn);
    bgp->processAnnouncements();

    // Second announcement from origin (should win)
    Announcement originAnn("192.168.1.0/24", {300}, 300, Relationship::ORIGIN);
    bgp->enqueueAnnouncement(originAnn);
    bgp->processAnnouncements();

    const auto &rib = bgp->getlocalRib();
    const Announcement &stored = rib.at("192.168.1.0/24");
    EXPECT_EQ(Relationship::ORIGIN, stored.getRelationship());
}
...
TEST_F(BGPTest, ConflictResolution_ProviderDoesNotBeatCustomer)
{
    ...
    Announcement providerAnn("192.168.1.0/24", {300, 400}, 300, Relationship::PROVIDER);
    bgp->enqueueAnnouncement(providerAnn);
    bgp->processAnnouncements();
...
TEST_F(BGPTest, ConflictResolution_SameRelationship_ShorterPathWins)
{
    ...
    Announcement longPath("192.168.1.0/24", {200, 300, 400}, 200, Relationship::CUSTOMER);
    bgp->enqueueAnnouncement(longPath);
    bgp->processAnnouncements();

    // Second announcement with shorter path (should win)
    Announcement shortPath("192.168.1.0/24", {500}, 500, Relationship::CUSTOMER);
...
TEST_F(BGPTest, ConflictResolution_SameRelationship_LongerPathLoses)
{
    ...
    Announcement shortPath("192.168.1.0/24", {200}, 200, Relationship::PEER);
    bgp->enqueueAnnouncement(shortPath);
    bgp->processAnnouncements();

    // Second announcement with longer path (should lose)
    Announcement longPath("192.168.1.0/24", {300, 400, 500}, 300, Relationship::PEER);
    bgp->enqueueAnnouncement(longPath);
    bgp->processAnnouncements();
...
TEST_F(BGPTest, ConflictResolution_SameRelationshipAndPath_LowerNextHopWins)
{
    ...
    Announcement highNextHop("192.168.1.0/24", {300}, 300, Relationship::CUSTOMER);
    bgp->enqueueAnnouncement(highNextHop);
    bgp->processAnnouncements();

    // Second announcement with lower next hop (should win)
    Announcement lowNextHop("192.168.1.0/24", {200}, 200, Relationship::CUSTOMER);
...
TEST_F(BGPTest, ConflictResolution_PathLengthOverridesNextHop)
{
    ...
    Announcement longPathLowNext("192.168.1.0/24", {100, 200, 300}, 100, Relationship::PEER);
    bgp->enqueueAnnouncement(longPathLowNext);
    bgp->processAnnouncements();

    // Second announcement: shorter path but higher next hop (should win due to path)
    Announcement shortPathHighNext("192.168.1.0/24", {900}, 900, Relationship::PEER);
...
{
    Announcement ann1("192.168.1.0/24", {200}, 200, Relationship::CUSTOMER);
    Announcement ann2("10.0.0.0/8", {300}, 300, Relationship::PEER);
    Announcement ann3("172.16.0.0/12", {400}, 400, Relationship::PROVIDER);
...
TEST_F(BGPTest, BatchProcessingMultiplePrefixes)
{
    ...
    bgp->enqueueAnnouncement(Announcement("192.168.1.0/24", {200}, 200, Relationship::CUSTOMER));
    bgp->enqueueAnnouncement(Announcement("10.0.0.0/8", {300}, 300, Relationship::PEER));
    bgp->enqueueAnnouncement(Announcement("172.16.0.0/12", {400}, 400, Relationship::PROVIDER));
...
TEST_F(BGPTest, ComplexConflictResolution_MultipleUpdates)
{
    ...
    bgp->enqueueAnnouncement(Announcement(prefix, {200}, 200, Relationship::PROVIDER));
    bgp->processAnnouncements();
    EXPECT_EQ(Relationship::PROVIDER, bgp->getlocalRib().at(prefix).getRelationship());

    // Second: peer announcement (should win - better relationship)
    bgp->enqueueAnnouncement(Announcement(prefix, {300, 400}, 300, Relationship::PEER));
    bgp->processAnnouncements();
    EXPECT_EQ(Relationship::PEER, bgp->getlocalRib().at(prefix).getRelationship());

    // Third: customer announcement (should win - best relationship)
    bgp->enqueueAnnouncement(Announcement(prefix, {500, 600, 700}, 500, Relationship::CUSTOMER));
    bgp->processAnnouncements();
    EXPECT_EQ(Relationship::CUSTOMER, bgp->getlocalRib().at(prefix).getRelationship());

    // Fourth: another peer announcement with shorter path (should lose - worse relationship)
    bgp->enqueueAnnouncement(Announcement(prefix, {800}, 800, Relationship::PEER));
    bgp->processAnnouncements();
    EXPECT_EQ(Relationship::CUSTOMER, bgp->getlocalRib().at(prefix).getRelationship());
...
TEST_F(BGPTest, RealisticPropagationScenario)
{
    ...
    bgp->enqueueAnnouncement(Announcement("203.0.113.0/24", {50}, 50, Relationship::CUSTOMER));

    // From peer AS 200 (medium path, medium relationship)
    bgp->enqueueAnnouncement(Announcement("198.51.100.0/24", {200, 250}, 200, Relationship::PEER));

    // From provider AS 1000 (various paths)
    bgp->enqueueAnnouncement(Announcement("192.0.2.0/24", {1000, 2000, 3000}, 1000, Relationship::PROVIDER));
...
TEST_F(BGPTest, SingleHopOriginAnnouncement)
{
    ...
    Announcement ann("192.168.1.0/24", {100}, 100, Relationship::ORIGIN);
    bgp->enqueueAnnouncement(ann);
    bgp->processAnnouncements();
